...
LSTM 회귀 문제 : 고장 날 때까지 얼마나 버틸 수 있는가?
'''
...
model_path = './data/pm_model/regression_model.h5'
...
print("label_array's shape ")
print(label_array.shape)
print('id = 1 일 때 라벨? ')
print(gen_labels(train_df[train_df['id']==1], sequence_length, ['RUL']))
import sys
sys.exit(1)
...
print( "look_back = %s, features = %s" % (sequence_length, nb_features))
model = Sequential()
model.add(LSTM(
...
plt.show()
fig_acc.savefig("./data/model_regression_r2.png")
...
plt.legend(['train', 'test'], loc='upper left')
plt.show()
fig_acc.savefig("./data/pm_regression_model_mae.png")
...
plt.legend(['train', 'test'], loc='upper left')
plt.show()
fig_acc.savefig("./data/pm_regression_model_loss.png")
...
test_set.to_csv('./data/submit_regression_train.csv', index = None)
...
if os.path.isfile(model_path):
    ...
    test_set.to_csv('./data/submit_regression_test.csv', index = None)
    ...
    plt.show()
    fig_verify.savefig("./data/pm_regression_model_verify.png")