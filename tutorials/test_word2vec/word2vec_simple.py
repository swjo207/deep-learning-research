...
def create_train_ds():
	...
	def gen():
		i = 0
		features_word = numpy.zeros((BATCH_SIZE,), 'int32')
		labels = numpy.zeros((BATCH_SIZE,), 'int32')
		while True:
			for j in range(BATCH_SIZE // n_neigh):
				s = j * n_neigh
				features_word[s:s + n_neigh] = data_word_ids[i + c]
				labels[s:s + n_neigh] = [data_word_ids[i + o] for o in random.sample(all_neigh_offsets, n_neigh)]
				i = (i + 1) % n_words
			yield ({'word': features_word}, labels)
...
def create_model(features, labels, mode):
	...
	word_vecs = tf.Variable(tf.random_uniform([VOC_SIZE, dim_embed], -1.0, 1.0), name = 'word_vecs')
	inputs = tf.nn.embedding_lookup(word_vecs, features['word'])
...
# pip install sklearn matplotlib scipy
...
plot_only = 500
word_vecs = es.get_variable_value('word_vecs')
