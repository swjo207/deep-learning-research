...
def create_train_ds():
	buf_size = TRAIN_BATCH_SIZE * 1000
	...
	return ds.apply(tf.contrib.data.group_by_window(
		...
		lambda key, ds_: ds_.padded_batch(TRAIN_BATCH_SIZE,
			((tf.TensorShape([None]), tf.TensorShape([])),
				(tf.TensorShape([None]), tf.TensorShape([None]), tf.TensorShape([]))),
			((EOS, 0), (EOS, EOS, 0))),
		window_size = TRAIN_BATCH_SIZE))
...
def create_test_ds():
	...
	def conv(r1):
		...
		return ((words1[r1[0]:r1[0] + n1], n1),)
	ds = ds.map(conv)
	return ds.padded_batch(TEST_BATCH_SIZE,
		((tf.TensorShape([None]), tf.TensorShape([])),),
		((EOS, 0),))
...
def create_model(features, labels, mode):
	...
	attention_mechanism = tf.contrib.seq2seq.LuongAttention(
		DIM_WORD_EMBED, encoder_out, memory_sequence_length = features_len)
	decoder_cell = tf.contrib.seq2seq.AttentionWrapper(
		create_rnn_cell(DECODER_NUM_SUBCELL),
		attention_mechanism,
		attention_layer_size = DIM_WORD_EMBED,
		alignment_history = mode == tf.estimator.ModeKeys.PREDICT)
	encoder_state_copy = decoder_cell.zero_state(batch_size, tf.float32).\
	...
	if mode == tf.estimator.ModeKeys.TRAIN:
		...
		decoder = tf.contrib.seq2seq.BasicDecoder(decoder_cell, helper, encoder_state_copy)
		with tf.variable_scope('decoder') as scope:
			decoder_out, _, _ = tf.contrib.seq2seq.dynamic_decode(decoder, swap_memory = True, scope = scope)
			#print(decoder_out.rnn_output)
		...
		with tf.variable_scope('decoder') as scope:
			decoder_out, decoder_state, _ = tf.contrib.seq2seq.dynamic_decode(decoder,
				maximum_iterations = features_max_len * 2, swap_memory = True, scope = scope)
		alignment = decoder_state.alignment_history.stack()
		alignment = tf.summary.image("alignment", tf.expand_dims(tf.transpose(alignment, [1, 2, 0]), -1) * 255)
		preds = {'predicted_sen': decoder_out.sample_id, 'alignment': alignment}
		return tf.estimator.EstimatorSpec(mode, predictions = preds)

cfg = tf.estimator.RunConfig(save_checkpoints_steps = 100)
args = {}
#args['config'] = cfg
es = tf.estimator.Estimator(create_model, model_dir = dir + '/model', **args)
...
for _ in range(30):
	...
	args = {}
	#args['steps'] = 100
	es.train(create_train_ds, hooks = [train_hook], **args)
	train_stats = train_hook.stats
	print('  begin to test')
	test_candis = []
	alignment = None
	for preds in es.predict(create_test_ds, yield_single_examples = False):
		for i, sen in enumerate(preds['predicted_sen']):
			if EOS in sen:
				sen = tuple(sen)
				sen = sen[:sen.index(EOS)]
			test_candis.append(sen)
		if alignment is None: alignment = preds['alignment']
	alignment_value = tf.Summary.FromString(alignment).value
	alignment_value[0].tag = 'alignment'
	...
	summary = tf.Summary()
	summary.value.add(tag = 'bleu', simple_value = bleu)
	summary.value.extend(alignment_value)
	writer.add_summary(summary, train_stats['global_step'])
	writer.flush()
