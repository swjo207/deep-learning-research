...
def create_model(data_format):
  ...
  return tf.keras.Sequential(
      [
          l.Reshape(input_shape, input_shape=(28 * 28,)),
          l.Conv2D(
              32,
              5,
              padding='same',
...
def main(argv):
  ...
  # batch_size: 100, train_epochs: 40, epochs_between_evals: 1
  # total number of step: 24000 = 40 * (60000 / 100)
  t = time.time()
  for i in range(flags.train_epochs // flags.epochs_between_evals):
    mnist_classifier.train(input_fn=train_input_fn, hooks=train_hooks)
    new_t = time.time()
    print('dt:%.1f, step:%d' % (new_t - t, (i+1) * (60000/flags.batch_size)))
    t = new_t
